...
module.exports = 
{
    ...
    run: (client, message, args) => 
    {
        ...
        if(!args[1])
        {
            return message.channel.send("Auf welchem Level?")
                    .then(msg => msg.delete({timeout:10000}));
        }
        ...
        try
        {
            //Setzt die XP von User fest
            xp[User.id].xp = Setzen;

            //Antwortet was geändert wurde
            return message.channel.send("**" + User.displayName + " XP** wurde auf **" + Setzen + " XP** gesetzt")
                    .then(msg => msg.delete({timeout:10000}));
        }
        catch
        {
